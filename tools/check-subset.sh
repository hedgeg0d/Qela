...
for f in srcql/*.qela std/*.qela; do
	...
	# Generated by tools/genblob.py. Scanning generated bytes for language
	# features says nothing about stage1's own sources, so skip it the way
	# std/* is skipped below.
	...
	case "$f" in
	std/*) ;;
	# Generated by tools/genblob.py: `$bytes` calls and two constants, no
	# stage1 code of its own, so it is exempt like std/* is.
	srcql/std_blob.qela) ;;
